...
async function compareInterface() {
  ...
  try {
    result.value = await api.compareInterface({ name: scenarioName.value.trim() || '接口工作台比较', oldRequest: oldRequest.value, newRequest: newRequest.value, options: options.value })
...
onMounted(() => {
  ...
  scenarioName.value = payload.name
...
<template>
  <div class="view interface-view">
    ...
    <section class="interface-workspace panel">
      ...
      <div class="scenario-row"><div><strong>比较场景名称/备注</strong><span>保存到历史记录后，可按此内容模糊查询并快速回填</span></div><el-input v-model="scenarioName" maxlength="100" show-word-limit clearable placeholder="例如：订单详情接口 V2 灰度验证" /></div>
