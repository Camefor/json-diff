...
// 把历史记录中的场景名称与接口请求快照写回 store，跳转后由 InterfaceView 消费回填
// 列表项触发时 target 只有摘要和 id，按需拉详情；详情抽屉触发时已带请求快照，直接复用
async function restoreInterface(target: { id: string; name: string; oldRequest?: InterfaceRequest | null; newRequest?: InterfaceRequest | null }) {
  ...
  appStore.setInterfaceRestore({ name: target.name, oldRequest, newRequest })
